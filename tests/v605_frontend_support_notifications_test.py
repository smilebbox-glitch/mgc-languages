...
for token in (
    ...
    "new root.Notification(item.title",
    "tag: 'mgc-learning-nudge'",
...
for name in ('renderNotifications', 'saveNudgeSettings', 'maybeBrowserNudge'):
    assert f'function {name}' in APP
assert len(APP.encode('utf-8')) < 150_000

for script in ('support_notifications.js', 'navigation.js', 'boot.js'):
    subprocess.run(['node', '--check', str(ROOT / 'static/frontend' / script)], check=True, cwd=ROOT)

print('PASS: v6.0.5 support-notifications owns nudge UI/actions with browser/support parity and staged legacy fallback')
