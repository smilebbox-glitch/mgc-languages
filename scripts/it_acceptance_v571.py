...
parser=argparse.ArgumentParser(description='MGC Languages v5.7.1 runtime IT acceptance')
...
check('liveness',st==200 and live.get('version')=='5.7.1',f"status={st} version={live.get('version')}")
...
check('schema head',(ready.get('checks') or {}).get('schema_head',{}).get('current')=='c57d0a31f570',str((ready.get('checks') or {}).get('schema_head')))
st,meta=jget('/api/meta')
check('meta/version',st==200 and meta.get('version')=='5.7.1',f"profile={meta.get('slo_profile')}")
...
if metrics_token:
    ...
    required=['mgc_slo_availability_percent','mgc_slo_error_rate_percent','mgc_slo_p95_ms','mgc_recovery_state','mgc_pilot_alerts_open','mgc_srs_cards_due','mgc_question_attempts_total','mgc_db_query_p95_ms','mgc_db_pool_saturation_percent','mgc_recovery_backup_age_minutes','mgc_recovery_restore_evidence_age_days']
...
if auth_mode=='local' and admin_user and admin_password:
    ...
    if admin_checks:
        ...
        learning_standard=found.get('learning_standard') or {}
        check('Putonghua primary scope',learning_standard.get('name')=='Путунхуа (普通话)' and 'основной' in (learning_standard.get('primary_message') or '').lower(),str(learning_standard.get('name')))
        check('dialects reference-only','только справка' in (learning_standard.get('reference_message') or '').lower(),learning_standard.get('reference_message',''))
        st,governance=jget('/api/admin/pilot/governance-summary')
        check('pilot governance summary',st==200 and all(k in governance for k in ('groups_total','groups_active','quotas','usage_today')),f"status={st} groups={governance.get('groups_total')}")
        st,groups=jget('/api/admin/pilot/groups')
        check('pilot groups endpoint',st==200 and isinstance(groups,list),f"status={st} groups={len(groups) if isinstance(groups,list) else 'n/a'}")
        st,features=jget('/api/admin/pilot/features')
        feature_keys={x.get('flag_key') for x in features} if isinstance(features,list) else set()
        check('feature flag catalog',st==200 and {'games','xp_economy','learning_nudges','chinese_reference','server_audio','ai_assistant'}.issubset(feature_keys),f"flags={sorted(feature_keys)}")
        ...
        check('v5.7.1 DB observability',st==200 and 'query_telemetry' in (dash.get('database') or {}) and 'pool' in (dash.get('database') or {}),f"status={st}")
        check('v5.7.1 recovery evidence dashboard',st==200 and 'recovery_evidence' in dash and 'rpo_rto' in dash,f"status={st}")
        st,dbtel=jget('/api/admin/database/telemetry')
        check('database telemetry endpoint',st==200 and 'queries' in dbtel and 'pool' in dbtel,f"status={st}")
        st,revidence=jget('/api/admin/recovery/evidence')
        check('recovery evidence endpoint',st==200 and all(k in revidence for k in ('backup','restore_rehearsal','objectives')),f"status={st}")
        st,system=jget('/api/admin/system/summary')
        check('v5.7 security profile',st==200 and system.get('rls_enabled') is True and system.get('term_approval_required') is True and 'otel' in system,f"status={st} rls={system.get('rls_enabled')}")
        st,chain=jget('/api/admin/audit/verify-chain')
        check('audit hash chain',st==200 and chain.get('ok') is True,f"events={chain.get('events')} broken={chain.get('broken_ids')}")
        st,qquality=jget('/api/admin/learning/question-quality')
        check('question quality telemetry',st==200 and 'items' in qquality and 'note' in qquality,f"items={len(qquality.get('items') or [])}")
...
if args.report:
    ...
    rows=['# MGC Languages v5.7.1 — IT Acceptance Report','',f'- Generated: {datetime.now(timezone.utc).isoformat()}',f'- Target: `{base}`',f'- Auth mode: `{auth_mode}`',f'- Result: **{passed} PASS / {failed} FAIL / {skipped} SKIP**','', '| Check | Result | Detail |','|---|---|---|']
    for name,status,detail in results:
        rows.append(f"| {name.replace('|','/')} | **{status}** | {str(detail).replace('|','/')} |")
    rows += ['','## Sign-off boundary','Passing automated checks supports a controlled pilot decision. Corporate OIDC, TLS/secure-cookie configuration, PostgreSQL FORCE RLS verification, backup/restore/PITR rehearsal, central monitoring and any required security review must still be evidenced on the real IT environment.','']
...
if failed:
    raise SystemExit(1)
print('PASS: v5.7.1 runtime IT acceptance')
