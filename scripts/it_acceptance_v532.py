...
jget('/health/live'); ready=jget('/health/ready'); meta=jget('/api/meta')
assert ready.get('checks',{}).get('schema_head',{}).get('status') == 'ok', ready
assert str(meta.get('version','')).startswith('5.3.2'),meta
...
print('PASS: v5.3.2 Putonghua/TTS/schema-readiness/IT acceptance checks')
