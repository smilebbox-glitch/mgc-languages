...
parser=argparse.ArgumentParser(description='MGC Languages v5.7 runtime IT acceptance')
...
if metrics_token:
    ...
    required=['mgc_slo_availability_percent','mgc_slo_error_rate_percent','mgc_slo_p95_ms','mgc_recovery_state','mgc_pilot_alerts_open','mgc_srs_cards_due','mgc_question_attempts_total']
...
if args.report:
    ...
    rows=['# MGC Languages v5.7 — IT Acceptance Report','',f'- Generated: {datetime.now(timezone.utc).isoformat()}',f'- Target: `{base}`',f'- Auth mode: `{auth_mode}`',f'- Result: **{passed} PASS / {failed} FAIL / {skipped} SKIP**','', '| Check | Result | Detail |','|---|---|---|']
...
if failed:
    raise SystemExit(1)
print('PASS: v5.7 runtime IT acceptance')
