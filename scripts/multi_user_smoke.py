...
def main() -> int:
    parser = argparse.ArgumentParser(description="Concurrent smoke for an already running MGC Languages LAN deployment")
    ...
    clients = max(1, min(200, args.clients))
    requests = max(1, min(10000, args.requests))
    base = args.base_url.rstrip("/")
    urls = [f"{base}/health" if i % 2 == 0 else f"{base}/api/meta" for i in range(requests)]
    ...
    if p95 > args.max_p95_ms:
        print(f"FAIL: p95 {p95:.2f} ms exceeds {args.max_p95_ms:.2f} ms")
        return 3
    print("PASS: concurrent LAN smoke completed without request failures")
    return 0
