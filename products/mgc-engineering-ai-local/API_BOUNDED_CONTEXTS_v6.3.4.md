# API Bounded Context Ownership — v6.3.4

Public paths remain under `/api/v1`. v6.3.4 preserves all 181 v6.2.0 legacy method/path contracts and adds 17 additive bounded-context routes through v6.3.x. Revision/conflict management does not rename legacy routes.
...
- Bounded-context routes: **198**.
- Legacy v6.2.0 method/path contracts preserved: **181/181**.
- Operations/admin routes are composed separately and are not counted in the six bounded-context total.
- Runtime profile composition remains an availability policy, not authorization.
- v6.3.4 conflict handling is human-reviewed; automatic merge of controlled engineering content remains disabled.
