# MGC Engineering AI Local v6.3.28 — Real Integration Certification

## Scope

This v6.3.28 copy preserves the certified adapter contract from v6.3.27 and is paired with `INTEGRATION_RUNTIME_ASSURANCE_v6.3.28.md` for bounded cache staleness and recovery-sync behavior.

v6.3.28 formalizes vendor-neutral certification for **PLM, PDM, ERP, MES and QMS** corporate gateways. It does not embed vendor SDKs or claim certification against a specific production system without target-host evidence.
...
v6.3.28 reuses and certifies the existing v6.0.2/v6.0.3 controls instead of duplicating them:
...
v6.3.28 certifies the software contract and provides the target-host test harness. A real integration is not production-certified until the company executes the live contract probe against its actual gateway, verifies reconciliation on representative engineering data, records authentication/TLS/network evidence and completes the existing human-controlled release process.
