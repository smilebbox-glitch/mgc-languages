# API Bounded Context Ownership — v6.3.6

Public paths remain under `/api/v1`. v6.3.6 preserves all 181 v6.2.0 legacy method/path contracts and adds enterprise identity/policy enforcement and a release manifest additively.
...
- Scoped identity policy CRUD for controlled actions.
- Controlled approval-delegation creation/list/revoke with no Engineering Admin delegation.
- Approval evidence stores sanitized identity + assurance snapshots.
- Release-package manifest endpoint returns canonical SHA-256 handover evidence and performs no production write-back.
- Optional PostgreSQL RLS foundation is limited to identity-policy/delegation tables; existing Project/Area/Document ACL remains application-enforced.
