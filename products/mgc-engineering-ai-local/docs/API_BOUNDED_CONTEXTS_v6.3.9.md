# API Bounded Context Ownership — v6.3.9

Public paths remain under `/api/v1`. v6.3.9 preserves all 181 v6.2.0 legacy method/path contracts and adds database performance/scale operations additively.
...
## v6.3.9 additive performance surfaces

- `GET /operations/performance` — admin-only privacy-safe DB/pool/query-budget/scale telemetry.
- `GET /audit/cursor` — admin-only keyset pagination for large audit trails; legacy `GET /audit` remains unchanged.

Runtime composition is still profile-aware and is not an authorization model. Project / Manufacturing Area / Document ACL remain authoritative.
