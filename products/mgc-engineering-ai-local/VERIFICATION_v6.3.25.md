# Verification — MGC Engineering AI Local v6.3.25
...
- DB migration: none
- Theme: External Trust Anchoring & Evidence Retention Governance

## Backend regression

Final inventory: **590 tests / 98 test files**.

Eight non-overlapping pytest groups were executed from repository root with `PYTHONPATH=backend` and a packaging-only `pytest_sessionfinish` hook used only to avoid the known third-party process-shutdown hang after pytest has completed.
...
Total: **590/590 PASS**. Failures/errors/skips: **0/0/0**.

## v6.3.25 trust/retention checks

- External Trust & Retention preflight: **32/32 PASS**
- specialized v6.3.25 tests: **15/15 PASS** (included in full regression)
- bounded key validity and expired-key rejection tested
- rotation-due status tested
- retention policy/no-auto-delete tested
- legal hold lifecycle tested
- checkpoint tip binding tested
- mismatched external anchor receipt rejected
- WORM receipt checkpoint/retention binding tested
- missing optional trust yields `CONDITIONAL`
- required missing trust fails closed
- offline auditor bundle export/replay tested
- auditor bundle tamper detection tested
- adapter execution confirmed `shell=False`

## Existing hardening gates

- Acceptance Evidence Registry: 33/33 PASS
...
- BUILD_INPUTS: 102 file inputs + 14 image refs
- 95 file inputs present/hashed
- 7 expected corporate lock/wheelhouse artifacts absent
- build input provenance: 102/102 verified
- status: **CONDITIONAL**
...
- real corporate WORM/Object Lock enforcement;
- external RFC3161/notary/transparency-log anchoring;
...
Final clean-tree payload: **953 files**. Final ZIP members: **954** including `BUILD_MANIFEST.json`. Independent ZIP→manifest verification: **953/953 PASS**; missing 0, extra 0, size mismatch 0, SHA-256 mismatch 0; `unzip -t` PASS.
