...
- standardized vehicle classes for passenger car, LCV, truck, bus, special vehicle and component programs;
- added canonical powertrain/drivetrain/wheelbase/axle/plant/cab/GVW/payload/battery configuration attributes over the existing VehicleVariant `attributes_json`;
- exposed a normalized configuration read-model while preserving legacy model-year/market/body/engine/transmission/trim fields;
- preserved explicit ConfigurationApplicability semantics: UNKNOWN is never treated as applies-to-all;
- extended the existing configuration UI for passenger and commercial vehicle variants without adding another workspace or API route;
- added passenger and 6x4/4x2 truck synthetic demo cases;
- added multi-vehicle applicability preflight;
- application 6.3.34 / DB schema 6.3.13; no migration; adjacent rollout 6.3.33 <-> 6.3.34.

# v6.3.33 — Pilot Readiness & UX Simplification

- reduced primary engineer navigation to five workspaces: Home, Projects, Parts/BOM, Work Instructions, Search/AI;
- added bounded evidence-backed Action Center derived from existing readiness blockers, with no new source of truth;
- added Project Focus Workspace for actions and direct work-object navigation;
- moved deep engineering modules behind progressive disclosure instead of deleting functionality;
- preserved separate IT/Admin surface and contextual access to Object 360, documents, checks and ECR/ECO;
- Action Center remains advisory-only and human-controlled;
- application 6.3.33 / schema 6.3.13; no DB migration; adjacent rollout window 6.3.32 ↔ 6.3.33.

# v6.3.32 — Resilience Certification & Recovery Baselines

- signed live resilience-drill evidence can be promoted into a human-approved recovery baseline;
- approved drill campaigns define mandatory scenarios and immutable RTO regression policy;
- per-scenario and aggregate RTO comparison detects ratio, absolute-delta and ceiling regressions;
- bootstrap/simulated evidence cannot satisfy release `GO`;
- detached OpenSSL SHA-256 signature verification and public-key fingerprint binding are fail-closed;
- rolling/blue-green rollout requires resilience certification `GO`; emergency rollback remains ungated;
- technical `GO` never sets `production_authorized=true`; human change/release approval remains mandatory;
- application 6.3.32 / schema 6.3.13; no DB migration; adjacent rollout window 6.3.31 ↔ 6.3.32.

# v6.3.31 — Resilience Drill Orchestration

- bounded allowlisted drill catalog for API replica, CPU worker, Redis, Qdrant and integration-gateway failure scenarios;
- plan-only default; live injection requires exact `DRILL` confirmation and never executes arbitrary shell from drill input;
- production dependency drills fail closed unless an explicit override and maintenance-window reference are present;
- mandatory recovery step from a `finally` boundary and bounded 1–120 second requested fault window;
- canonical `mgc.resilience-drill-plan.v1` and `mgc.resilience-drill-evidence.v1` SHA-256 contracts;
- simulation can produce only `SIMULATED`, never production-grade `PASS`;
- privacy-safe binding to v6.3.30 incident evidence by SHA-256 + signal codes only; raw signal payloads are not copied and root cause is never auto-asserted;
- `mgcctl drill catalog|plan|run|bind|validate` plus full preflight integration;
- application 6.3.31 / schema 6.3.13; no DB migration; adjacent rollout window 6.3.30 ↔ 6.3.31.
...
- privacy-safe target-host probe for Linux CPU/RAM/disk/resource-limit/Docker/Compose/time-sync/storage-fsync evidence;
- deterministic baseline envelope profiles for 15/30/100 users without claiming workload capacity;
- one-to-one topology node evidence coverage with missing/extra/duplicate fail-closed behavior;
- canonical SHA-256 report integrity and optional detached OpenSSL signature;
- `mgcctl certify target-host` and `target_host_deployment_guard.py`;
- rolling and blue/green rollout require a PASS target-host report; rollback remains unblocked for emergency safety;
- production authorization, load/SLO, CVE/SCA, integration certification and human change approval remain separate gates;
- application 6.3.29 / schema 6.3.13; no DB migration.

# v6.3.28 — Integration Runtime Assurance

- bounded cache staleness for PLM/PDM/ERP/MES/QMS integration data;
- stale cached reads fail closed with explicit runtime posture;
- opt-in recovery sync breaks source-health/cache deadlocks without enabling writeback;
- successful `IntegrationRun.status=ok` is counted correctly in reliability metrics;
- application 6.3.28 / schema 6.3.13; no DB migration.

# v6.3.27 — Real Integration Certification

- vendor-neutral PLM/PDM/ERP/MES/QMS certification profiles and reference contracts;
- static fail-closed security/transport/writeback contract checks;
- bounded read-only live probe for deterministic repeated reads, schema validity, stable IDs, replay keys and checkpoints;
- privacy-safe SHA-256 sample evidence;
- ACTIVE / DEGRADED_READ_ONLY / BLOCKED integration posture with optional fail-closed sync enforcement;
- Engineering Admin integration certification/runtime-posture endpoints;
- `mgcctl certify integration`, contract-suite CLI and source preflight;
- application 6.3.27 / schema 6.3.13; no DB migration.

# v6.3.26 — Operations Consolidation & mgcctl

- unified operator-facing `mgcctl` facade for status/verify/deploy/rollback/backup/restore/certify/provenance/diagnose;
- old Makefile/script entry points preserved;
- destructive workflows retain exact confirmation tokens and safe argv delegation;
- application 6.3.26 / schema 6.3.13; no DB migration.

# v6.3.25 — External Trust Anchoring & Evidence Retention Governance

- bounded signing-key validity windows with rotation warnings and expiry enforcement for new evidence;
- historical signature verification preserves evidence that was valid before later key expiry/revocation;
- append-only minimum-retention policy and immutable legal-hold placement/release events;
- verified registry checkpoints bind sequence/tip SHA-256, retention posture and key lifecycle;
- external tip-hash anchor adapter uses JSON argv + `shell=False` and rejects mismatched receipts;
- WORM/Object-Lock adapter seals checkpoint objects and rejects shorter-than-requested retention;
- governance status distinguishes PASS / CONDITIONAL / FAIL and never self-authorizes production;
- offline auditor bundle contains full event/object chain, audit/governance reports and per-file SHA-256 manifest, with private keys excluded;
- application 6.3.25 / schema 6.3.13; no DB migration.

# v6.3.24 — Acceptance Evidence Registry & Release Provenance Ledger

- append-only sequence + parent-hash provenance event ledger;
- content-addressed SHA-256 object store for release evidence and detached signatures;
- exclusive append locking and fail-closed symlink/overwrite checks;
- public-key registration, rotation and revocation; private keys are rejected;
- cryptographic replay verifies signed acceptance/baseline/load evidence against key state at registration sequence;
- cross-version p95/p99/error-rate/throughput/DB-pool/RTO/RPO comparison and consolidated audit report;
- optional automatic registry append from release acceptance and baseline promotion;
- Engineering Admin privacy-safe provenance summary;
- application 6.3.24 / schema 6.3.13; no DB migration; production authorization remains human-controlled.
...
- Added unified `mgcctl` operator CLI for status/verify/deploy/rollback/backup/restore/certify/provenance/diagnose.
- Preserved existing scripts and Makefile targets as backward-compatible canonical implementations.
- Added exact confirmation tokens, dry-run/JSON contracts, shell-free delegation, offline status and privacy-safe diagnostics.
- Advanced adjacent rolling/blue-green contract to 6.3.25 → 6.3.26 with schema 6.3.13 unchanged.
- Added 15 tests and 31-check Operations Consolidation preflight.
